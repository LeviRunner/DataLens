...
│   ├── .gitkeep
│   └── exemplos/
│       ├── finance.db                # banco SQLite: ativos + cotações + indicadores (Fase 1)
│       ├── acoes_b3.csv              # dataset estrela: ações B3 (PETR4/VALE3)   (Fase 1)
│       ├── acoes_eua.csv             # ações EUA (Apple/Tesla)                   (Fase 1)
│       ├── selic.json                # série 11 do BCB crua, como a API devolve  (Fase 2)
...
│           ├── json_connector.py     # lê JSON: arquivo local OU url pública     (Fase 2)
│           ├── sql_connector.py      # roda a QUERY do usuário no banco          (Fase 1)
│           └── api_connector.py      # API com chave/auth (JSON)                 (Fase 3)
├── app/
...
├── tests/
│   ├── __init__.py
│   ├── test_connectors.py            # testes de base.py + SQLConnector          (Fase 1)
│   ├── test_json_connector.py        # testes do conector JSON                   (Fase 2)
