...
CREATE TABLE assets (
    ...
    exchange TEXT            -- Stock exchange where it trades
);
...
-- Create an index on the 'date' column in 'quotes'
...
CREATE TABLE series (
    ...
    unit TEXT,                  -- Unit of measurement (e.g., %, BRL/USD)
    frequency TEXT              -- Update frequency (e.g., daily, monthly)
);

-- Create the 'indicators' table (formerly 'indicadores')
CREATE TABLE indicators (
    code INTEGER,               -- Foreign key referencing the series code
    date TEXT,                  -- Date in ISO format (yyyy-mm-dd)
    value REAL NOT NULL,        -- Observed value for that date
